...
shared (_init_msg) actor class NftPoC() = this {
    ...
    let icrc7_state_current = switch (icrc7_migration_state) {
        case (#v0_1_0(#data(state))) state;
        case (#v0_1_0(#id)) D.trap("Unexpected migration state: v0_1_0 #id");
        case (#v0_0_0(_)) D.trap("Unexpected migration state: v0_0_0");
    };
    ...
    let icrc37_state_current = switch (icrc37_migration_state) {
        case (#v0_1_0(#data(state))) state;
        case (#v0_1_0(#id)) D.trap("Unexpected migration state: v0_1_0 #id");
        case (#v0_0_0(_)) D.trap("Unexpected migration state: v0_0_0");
    };
    ...
    let icrc3_state_current = switch (icrc3_migration_state) {
        case (#v0_1_0(#data(state))) state;
        case (#v0_1_0(#id)) D.trap("Unexpected migration state: v0_1_0 #id");
        case (#v0_0_0(_)) D.trap("Unexpected migration state: v0_0_0");
    };
    ...
    public shared (msg) func icrc37_revoke_collection_approvals<system>(args : [RevokeCollectionApprovalArg]) : async [?RevokeCollectionApprovalResult] {
    ...
    public shared (msg) func icrc7_mint(tokens : ICRC7.SetNFTRequest) : async [ICRC7.SetNFTResult] {
        // TODO: Currently allowing anyone to mint temporarily, need to modify to enable minting based on permissions
        let owner = icrc7().get_state().owner;

        switch (icrc7().set_nfts<system>(owner, tokens, true)) {
            case (#ok(val)) val;
            case (#err(err)) D.trap(err);
        };
    };

    public shared (msg) func icrc7_burn(tokens : ICRC7.BurnNFTRequest) : async ICRC7.BurnNFTBatchResponse {
        // TODO: Currently allowing anyone to burn temporarily, need to modify to enable burning based on permissions
        let owner = icrc7().get_state().owner;

        switch (icrc7().burn_nfts<system>(owner, tokens)) {
            case (#ok(val)) val;
            case (#err(err)) D.trap(err);
        };
    };
